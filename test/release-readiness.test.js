...
test('package, installer and lockfile versions stay synchronized for v0.1.4', () => {
  const packageJson = JSON.parse(read('package.json'));
  const packageLock = JSON.parse(read('package-lock.json'));

  assert.equal(packageJson.version, '0.1.4');
  assert.equal(packageJson.shortVersionWindows, '0.1.4');
  assert.equal(packageJson.build.buildVersion, '0.1.4');
...
test('public release documentation and compliance artifacts are complete', () => {
  ...
  assert.equal(require('../package.json').shortVersionWindows, '0.1.4');
  assert.equal(require('../package.json').build.buildVersion, '0.1.4');
  ...
  // 按文档出现顺序断言 v0.1.4 的主线：版本统一、安装、标题栏与滚动条。
  assert.match(
    read('RELEASE_NOTES.md'),
    /v0\.1\.4[\s\S]*deepseek-v4-flash-vision-exp[\s\S]*deepseek-v4-pro[\s\S]*原子事件[\s\S]*时差[\s\S]*公众号/
  );
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /cheerio@1\.2\.0/);
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /摘星阁 \(Star-Picking-Pavilion\) 0\.1\.4/);
...
test('version verifier matches package, tag, installer and latest metadata', async t => {
  ...
  await fs.promises.writeFile(path.join(directory, 'Star-Picking-Pavilion-Setup-0.1.4.exe'), 'fixture');
  ...
  assert.deepEqual(verifyVersion({
    ...
    tag: 'v0.1.4',
    installer: 'Star-Picking-Pavilion-Setup-0.1.4.exe'
  });
...
test('v0.1.4 updater metadata uses the public version without a bridge', () => {
  const { prepareUpdateMetadata } = require('../scripts/prepare-update-metadata');
  const result = prepareUpdateMetadata({ packageJson: require('../package.json'), latestContent: 'version: 0.1.4\n' });
  assert.equal(result.version, '0.1.4');
  assert.equal(result.bridged, false);
  assert.equal(result.content, 'version: 0.1.4\n');
});
