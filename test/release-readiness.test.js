...
test('package and lockfile versions stay synchronized at v0.0.5', () => {
...
test('public release documentation and compliance artifacts are complete', () => {
  ...
  assert.equal(require('../package.json').version, '0.0.5');
  ...
  assert.match(read('RELEASE_NOTES.md'), /v0\.0\.5[\s\S]*键盘快捷键[\s\S]*对比度[\s\S]*未签名/);
...
test('third-party notices use the platform line ending', () => {
  ...
  const output = renderNotices({ version: '0.0.5' }, []);
...
test('version verifier matches package, tag, installer and latest metadata', async t => {
  ...
  await fs.promises.writeFile(path.join(directory, 'latest.yml'), 'version: 0.0.5\n');
  await fs.promises.writeFile(path.join(directory, 'Star-Picking-Pavilion-Setup-0.0.5.exe'), 'fixture');
  ...
  assert.deepEqual(verifyVersion({
    ...
    version: '0.0.5',
    tag: 'v0.0.5',
    installer: 'Star-Picking-Pavilion-Setup-0.0.5.exe'
  });
