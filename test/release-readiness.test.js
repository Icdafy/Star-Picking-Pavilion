...
test('package and lockfile versions stay synchronized at v0.0.8', () => {
...
test('public release documentation and compliance artifacts are complete', () => {
  ...
  assert.equal(require('../package.json').version, '0.0.8');
  ...
  // 按文档出现顺序断言 v0.0.8 的四条主线：字体 → 字号 → 缩放档位 → 未签名提示
  assert.match(read('RELEASE_NOTES.md'), /v0\.0\.8[\s\S]*思源黑体[\s\S]*Times New Roman[\s\S]*字号[\s\S]*缩放[\s\S]*未签名/);
...
test('README documents installation, privacy, recovery and security truthfully', () => {
  ...
  for (const required of [
    /Windows 10\/11.*x64/,
    /Star-Picking-Pavilion-Setup-0\.0\.8\.exe/,
    /SmartScreen/,
    /Get-FileHash/,
    /云幄\s*·\s*常用网址/,
    /%APPDATA%\\摘星阁/,
...
test('version verifier matches package, tag, installer and latest metadata', async t => {
  ...
  await fs.promises.writeFile(path.join(directory, 'latest.yml'), 'version: 0.0.8\n');
  await fs.promises.writeFile(path.join(directory, 'Star-Picking-Pavilion-Setup-0.0.8.exe'), 'fixture');
  ...
  assert.deepEqual(verifyVersion({
    ...
    version: '0.0.8',
    tag: 'v0.0.8',
    installer: 'Star-Picking-Pavilion-Setup-0.0.8.exe'
  });
