...
test('package and lockfile versions stay synchronized at v0.0.16', () => {
...
test('public release documentation and compliance artifacts are complete', () => {
  ...
  assert.equal(require('../package.json').version, '0.0.16');
  ...
  // 按文档出现顺序断言 v0.0.16 前端质感深化的主线：深化内容、卡片压边、
  // 流星与扫光两块新动效、以及安装提示。
  assert.match(
    read('RELEASE_NOTES.md'),
    /v0\.0\.16[\s\S]*质感[\s\S]*压边[\s\S]*流星[\s\S]*扫光[\s\S]*未签名/
  );
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /cheerio@1\.2\.0/);
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /摘星阁 \(Star-Picking-Pavilion\) 0\.0\.16/);
...
test('README documents installation, privacy, recovery and security truthfully', () => {
  ...
  for (const required of [
    /Windows 10\/11.*x64/,
    /Star-Picking-Pavilion-Setup-0\.0\.16\.exe/,
    /SmartScreen/,
    /Get-FileHash/,
    /云幄\s*·\s*常用网址/,
    /%APPDATA%\\摘星阁/,
...
test('version verifier matches package, tag, installer and latest metadata', async t => {
  ...
  await fs.promises.writeFile(path.join(directory, 'latest.yml'), 'version: 0.0.16\n');
  await fs.promises.writeFile(path.join(directory, 'Star-Picking-Pavilion-Setup-0.0.16.exe'), 'fixture');
  ...
  assert.deepEqual(verifyVersion({
    ...
    version: '0.0.16',
    tag: 'v0.0.16',
    installer: 'Star-Picking-Pavilion-Setup-0.0.16.exe'
  });
