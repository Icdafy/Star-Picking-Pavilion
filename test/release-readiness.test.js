...
test('package and lockfile versions stay synchronized for v0.1.0.2', () => {
  const packageJson = JSON.parse(read('package.json'));
  const packageLock = JSON.parse(read('package-lock.json'));

  assert.equal(packageJson.version, '0.1.2');
  assert.equal(packageJson.build.buildVersion, '0.1.0.2');
...
test('public release documentation and compliance artifacts are complete', () => {
  ...
  assert.equal(require('../package.json').version, '0.1.2');
  assert.equal(require('../package.json').build.buildVersion, '0.1.0.2');
  ...
  // 按文档出现顺序断言 v0.1.0.2 的主线：滚动安全区、日期玻璃与主题配色。
  assert.match(
    read('RELEASE_NOTES.md'),
    /v0\.1\.0\.2[\s\S]*标题栏[\s\S]*液态玻璃[\s\S]*配色[\s\S]*兼容/
  );
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /cheerio@1\.2\.0/);
  assert.match(read('THIRD_PARTY_NOTICES.txt'), /摘星阁 \(Star-Picking-Pavilion\) 0\.1\.0\.2/);
...
test('version verifier matches package, tag, installer and latest metadata', async t => {
  ...
  await fs.promises.writeFile(path.join(directory, 'latest.yml'), 'version: 0.1.2\n');
  await fs.promises.writeFile(path.join(directory, 'Star-Picking-Pavilion-Setup-0.1.0.2.exe'), 'fixture');
  ...
  assert.deepEqual(verifyVersion({
    ...
    version: '0.1.2',
    releaseVersion: '0.1.0.2',
    tag: 'v0.1.0.2',
    installer: 'Star-Picking-Pavilion-Setup-0.1.0.2.exe'
  });
