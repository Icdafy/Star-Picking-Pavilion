...
test('package and installer metadata use the v0.0.14 canonical identity', () => {
  assert.equal(pkg.name, 'star-picking-pavilion');
  assert.equal(pkg.version, '0.0.14');
...
test('release documentation points only to the canonical repository and installer', () => {
  ...
  assert.match(readme, /Star-Picking-Pavilion-Setup-0\.0\.14\.exe/);
  assert.match(releasing, /Star-Picking-Pavilion-Setup-0\.0\.14\.exe/);
