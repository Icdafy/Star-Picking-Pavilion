...
test('package and installer metadata use the v0.1.3 canonical identity', () => {
  assert.equal(pkg.name, 'star-picking-pavilion');
  assert.equal(pkg.version, '0.1.3');
  assert.equal(pkg.shortVersionWindows, '0.1.3');
  assert.equal(pkg.build.buildVersion, '0.1.3');
...
test('release documentation points only to the canonical repository and installer', () => {
  ...
  assert.match(readme, /Star-Picking-Pavilion-Setup-0\.1\.3\.exe/);
  assert.match(releasing, /Star-Picking-Pavilion-Setup-0\.1\.3\.exe/);
