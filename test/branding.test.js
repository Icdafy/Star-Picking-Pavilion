...
test('package and installer metadata use the v0.1.2 canonical identity', () => {
  assert.equal(pkg.name, 'star-picking-pavilion');
  assert.equal(pkg.version, '0.1.4');
  assert.equal(pkg.shortVersionWindows, '0.1.2');
  assert.equal(pkg.build.buildVersion, '0.1.2');
...
test('release documentation points only to the canonical repository and installer', () => {
  ...
  assert.match(readme, /Star-Picking-Pavilion-Setup-0\.1\.2\.exe/);
  assert.match(releasing, /Star-Picking-Pavilion-Setup-0\.1\.2\.exe/);
