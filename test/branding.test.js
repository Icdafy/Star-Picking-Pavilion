...
test('package and installer metadata use the v0.0.18 canonical identity', () => {
  assert.equal(pkg.name, 'star-picking-pavilion');
  assert.equal(pkg.version, '0.0.18');
...
test('release documentation points only to the canonical repository and installer', () => {
  ...
  assert.match(readme, /Star-Picking-Pavilion-Setup-0\.0\.18\.exe/);
  assert.match(releasing, /Star-Picking-Pavilion-Setup-0\.0\.18\.exe/);
