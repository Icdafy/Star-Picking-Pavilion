...
```powershell
npm ci
npm run verify:version -- --tag v0.0.8
npm test
npm run test:e2e
...
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.0.8.exe
```

v0.0.8 的签名状态预期为 `NotSigned`。ASAR 必须小于 18 MiB，安装包必须小于 120 MiB（v0.0.8 起随包内置思源黑体，约 4.6 MiB）。
...
- `Star-Picking-Pavilion-Setup-0.0.8.exe.blockmap`
- `latest.yml`
- `SHA256SUMS.txt`
- `sbom.cdx.json`
