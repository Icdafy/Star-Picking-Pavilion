...
```powershell
npm ci
npm run verify:version -- --tag v0.0.12
npm test
npm run test:e2e
...
npm run verify:version -- --tag v0.0.12 --artifacts
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.0.12.exe
```

v0.0.12 的签名状态预期为 `NotSigned`。ASAR 不得超过 12,476,662 字节，安装包不得超过 99,328,923 字节；两项均不得大于 v0.0.11 实测基线。思源黑体约 4.6 MiB，继续完整保留。
