...
```powershell
npm ci
npm run verify:version -- --tag v0.0.7
npm test
npm run test:e2e
...
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.0.7.exe
```

v0.0.7 的签名状态预期为 `NotSigned`。ASAR 必须小于 12 MiB，安装包必须小于 110 MiB。
...
- `Star-Picking-Pavilion-Setup-0.0.7.exe.blockmap`
- `latest.yml`
- `SHA256SUMS.txt`
- `sbom.cdx.json`
