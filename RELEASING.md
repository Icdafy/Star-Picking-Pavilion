...
```powershell
npm ci
npm run verify:version -- --tag v0.1.2
npm test
npm run test:e2e
...
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.1.2.exe
```

v0.1.2 的签名状态预期为 `NotSigned`。
...
自动更新比较版本采用单调递增的 SemVer；公开发布号、tag、安装包名与 Windows 文件/产品版本使用维护者指定的三段式或四段式版本。v0.1.2 对应内部 SemVer `0.1.4`：`latest.yml` 写 `0.1.4` 供 electron-updater 从 v0.1.1 的内部版本 `0.1.3` 正确判断升级，`package.json` 的 `build.buildVersion`、`shortVersionWindows`、tag 与安装包名写 `0.1.2`。应用界面必须从 UpdateInfo 的 GitHub tag/title 提取公开版本，禁止直接展示内部 `version`。创建 tag 前先运行版本校验：

```powershell
npm run verify:version -- --tag v0.1.2
git tag -a v0.1.2 -m "摘星阁 v0.1.2"
...
- `Star-Picking-Pavilion-Setup-0.1.2.exe.blockmap`
- `latest.yml`
- `SHA256SUMS.txt`
- `sbom.cdx.json`
