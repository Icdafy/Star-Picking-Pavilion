...
```powershell
npm ci
npm run verify:version -- --tag v0.0.15
npm test
npm run test:e2e
...
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.0.15.exe
```

v0.0.15 的签名状态预期为 `NotSigned`。
...
```powershell
npm run verify:version -- --tag v0.0.15
git tag -a v0.0.15 -m "摘星阁 v0.0.15"
...
- `Star-Picking-Pavilion-Setup-0.0.15.exe.blockmap`
- `latest.yml`
- `SHA256SUMS.txt`
- `sbom.cdx.json`
