...
```powershell
npm ci
npm run verify:version -- --tag v0.1.6
npm test
npm run test:e2e
...
npm run verify:version -- --tag v0.1.6 --artifacts
Get-AuthenticodeSignature .\dist\Star-Picking-Pavilion-Setup-0.1.6.exe
```

v0.1.6 的签名状态预期为 `NotSigned`。
...
- `Star-Picking-Pavilion-Setup-0.1.6.exe.blockmap`
- `latest.yml`
- `SHA256SUMS.txt`
- `sbom.cdx.json`
