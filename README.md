...
- 官方 DeepSeek 端点按任务分配：预筛、图片筛选、常规结构化使用 `deepseek-v4-flash-vision-exp`；重大政策、首飞适航、融资事故与复杂多事件研判使用 `deepseek-v4-pro` 的 high 思考模式。旧 Flash 配置自动迁移，自定义兼容端点保持所填模型。
- 相关资讯补取公开正文，最多分析 4 张图片（每张 4 MiB），先检查实际格式再以内联图片提交；有用实物、零部件、图纸与图表附于新闻下方，保留原文入口和 AI 解读标识。失败不阻断文字分析，图片不作为独立来源或发生日期证明。
- 原子事件保留主体、动作、客体、日期、完成/计划/失败状态与原文逐字证据。不同日期与不同状态禁止合簇；最近2万条已采纳记录中的事件参与交叉核实。官方一手（T1 且原站）或至少两个独立媒体域名的一致日期证据才能确认。已识别转载合并计数，来源不明的聚合结果与未识别账号的公众号不算独立来源；无法判定的消息显示待核实。
- 卡片右上角显示事件日期与报道日期的北京时间自然日差，并可展开核验原文。无日期证据显示「时差待核」。确认后的事件日期用于信息流排序与热度衰减，日报仍按采集窗口留存迟报旧闻。已有归档不覆写；升级后最近30天内最多200条已采纳消息渐进重判；其余旧数据未经本版分析时显示待核实。
- 新增 18 路双行业产业链检索入口，覆盖空域、适航、无人机物流、火箭发动机、卫星载荷与测控等；公众号使用公开订阅入口持续发现文章。
...
从 [GitHub Releases](https://github.com/Icdafy/Star-Picking-Pavilion/releases) 下载 `Star-Picking-Pavilion-Setup-0.1.4.exe`，双击并按向导安装。v0.1.4 尚未进行代码签名，因此 Windows SmartScreen 可能显示“Windows 已保护你的电脑”；请先核对校验值，再选择“更多信息 → 仍要运行”。
...
v0.0.14 把情报处理补成完整的八段式管线：结构化、数据清洗、预筛、标注、实体提取、原子事件分离、聚类、语义合并。入库前规范化地址与标题正文，同一篇文章从不同入口进来只落库一条；实体走词库与模型双通道合并并做别名归一，点击卡片上的实体即按它检索；一条资讯里的多件事被拆成「主体 · 动作 · 客体」，同一件事的不同措辞落到同一个事件键，字面相似度并不到的报道由语义合并补齐。该版本曾采用单模型；v0.1.4 已升级为下述双模型分工。
...
- 可选的 AI 五维评分、摘要与研判（Flash Vision 常规图文分析 + Pro 复杂思考，或任意 OpenAI 兼容服务）
