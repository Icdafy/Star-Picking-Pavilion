...
写入先在同级临时目录完成，校验清单最后生成，再以一次目录重命名提交。完整的已有日期会直接跳过；若发现同名档案不完整或被修改，原目录会保留，并写入带“补存”时间戳的新目录，不静默覆盖证据。应用启动、系统唤醒或解锁后会按日期顺序补齐遗漏周期。要准时保存，摘星阁需要在 08:00 运行，或之后再次启动；若开启“关闭主窗口后在后台运行”和登录自启，可减少漏存。
