...
从 [GitHub Releases](https://github.com/Icdafy/Star-Picking-Pavilion/releases) 下载 `Star-Picking-Pavilion-Setup-0.0.12.exe`，双击并按向导安装。v0.0.12 尚未进行代码签名，因此 Windows SmartScreen 可能显示“Windows 已保护你的电脑”；请先核对校验值，再选择“更多信息 → 仍要运行”。

下载 `SHA256SUMS.txt` 后，可以在 PowerShell 中验证安装包：

```powershell
Get-FileHash -Algorithm SHA256 .\Star-Picking-Pavilion-Setup-0.0.12.exe
...
清理每天在日报之后 20 分钟自动执行，也可以随时手动触发。到期条目连同 FTS5 全文索引行一起删除，成员不足的事件簇随之收敛，随后运行 SQLite 索引优化并截断 WAL。日报是自包含快照，保留期独立且更长。**已加星标的情报和情报备忘不参与清理，无论保留天数设成多少都会保留。**
