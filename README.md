...
后四项是模型看不到、代码却算得出的信号，且都有限幅，堆词、堆信源刷不满分。技术突破只在低空经济、商业航天的允许分类中生效：标题或摘要必须同时出现明确技术对象和“完成、首飞、验证、交付、量产”等已实现动作，并通过 T1/T1.5 信源或多源印证门槛；“计划、拟、或将、概念、传闻”以及营销噪声不会获得加成。卡片上的“技术突破”标识会说明命中的证据，规则集中在 `config/breakthroughs.json`，便于持续审计与迭代。
