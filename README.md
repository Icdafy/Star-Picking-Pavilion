...
摘星阁是一款面向低空经济与商业航天信息的 Windows 桌面情报站。它在本机聚合信源、检索与聚类文章，并可选择使用用户配置服务中的 Flash Vision 模型进行五维评分和情报研判。
...
接口依据：[DeepSeek 图像理解](https://api-docs.deepseek.com/zh-cn/guides/vision/)与[首次调用 API](https://api-docs.deepseek.com/zh-cn/)。启用 AI 后，相关新闻正文及经筛选的图片会发送给配置的模型服务。
...
从 [GitHub Releases](https://github.com/Icdafy/Star-Picking-Pavilion/releases) 下载 `Star-Picking-Pavilion-Setup-0.1.6.exe`，双击并按向导安装。v0.1.6 尚未进行代码签名，因此 Windows SmartScreen 可能显示“Windows 已保护你的电脑”；请先核对校验值，再选择“更多信息 → 仍要运行”。
...
v0.0.9 将桌面窗口的最小可用尺寸调整为 800×600，并让顶栏、主导航、筛选器、信源、设置、常用网址和日报按实际内容宽度完整自适应重排。小 / 标准 / 大 / 特大四档缩放都会提前触发对应的降栏布局，不使用横向导航滚动。
...
v0.0.14 把情报处理补成完整的八段式管线：结构化、数据清洗、预筛、标注、实体提取、原子事件分离、聚类、语义合并。入库前规范化地址与标题正文，同一篇文章从不同入口进来只落库一条；实体走词库与模型双通道合并并做别名归一，点击卡片上的实体即按它检索；一条资讯里的多件事被拆成「主体 · 动作 · 客体」，同一件事的不同措辞落到同一个事件键，字面相似度并不到的报道由语义合并补齐。该版本曾采用单模型；v0.1.5 起统一使用 Flash Vision 模型。

v0.0.16 只做前端质感深化（设计系统 v5）：卡片悬停浮起领域色渐变压边并叠同色系远光，精选卡底色向旗标色微倾、左缘书签拉通全高；深空主题偶有流星划过；技术突破胶囊配周期性缎面扫光，精选胶囊与五维进度条尽头带余辉；星标回弹、主按钮扫光、标题逐行下划线等微交互全部接入 prefers-reduced-motion。信息架构、功能、数据与设置全部不变。
...
v0.1.3 将 npm/Electron、安装器、Windows 文件属性、tag、Release 和安装包名全部统一为公开版本 `0.1.3`，并用一次性更新桥兼容已安装成内部 `0.1.5` 的 v0.1.2。安装只在明确点击后以静默 NSIS 执行，不再在失败后借普通退出悄悄完成。窗口顶部新增会折射流体背景的全宽液态玻璃层，最右侧与内嵌滚动条也直接使用 Aqua 深浅主题强调色。

## 主要功能

- 精选、全部动态与星标信息流
- 低空经济、商业航天领域和分类筛选
- 131 个内置信源：官方一手、央媒与科技媒体、交易所公告、关键词检索线、公众号与外媒接入位
...
- 可选的 AI 五维评分、摘要与研判（Flash Vision 图文分析，支持提供该模型的兼容服务）
...
`config/lexicon.json` 定义了这两个领域的核心词：18 个分组、268 个词条、739 个匹配面，每条带权重（w=10 领域定义词，w=6~9 核心主题与具名主体，w=2~5 相关概念）与别名。它是一份事实、三处使用：
...
词库加成和噪声惩罚均有限幅；关联报道数量不作为核验或评分依据。

技术突破不改变质量分，只影响热度。它仅在低空经济、商业航天的允许分类中生效：标题或摘要必须在同一语句中同时出现明确技术对象和“完成、首飞、验证、交付、量产”等已实现动作，并通过 T1/T1.5 信源与模型可信度门槛；“计划、拟、或将、概念、传闻”以及营销噪声不会获得加成。卡片上的“技术突破”标识会说明命中的证据，规则集中在 `config/breakthroughs.json`，便于持续审计与迭代。

热度按时间继续衰减，用于日报「当日热点」排序；可信技术突破会同时提高起始热度并适度延长半衰期：
...
信源分三级：T1 官方一手、T1.5 官方次级渠道、T2 媒体与聚合检索，等级直接进入打分系数。关键词检索线（`eastmoney://关键词`）默认相关性优先并叠加一路时效补充，可按 `?pages=2` 翻页；公告与电报适配器还支持 `cninfo://`、`sse://`、`szse://`、`cls://`。标题与摘要都不沾关键词、也不落在同领域词库内的泛资讯会在入库前被丢弃；已知不可达或依赖登录态、自建服务的接入位默认停用。

微信公众号使用新增的「公众号安全采集」类型：可添加公开 mp.weixin.qq.com/s/ 文章链接，或填写公开 Wechat2RSS/RSSHub 订阅地址。订阅用于持续发现文章，直接链接仅跟踪该篇；不枚举账号、不借用登录 Cookie、不绕过验证码。每次最多读取 5 篇，公众号请求间隔 3 秒，遇验证/拒绝暂停一小时并叠加信源退避。正文遵守 robots.txt，过滤后只收低空经济和商业航天。内置公众号接入位需要自行配置可访问的订阅实例，默认停用，不能声称覆盖全部公众号。
...
- **整份导出**：日报可「复制日报」得到纯文本（直接贴进微信群或邮件），也可「导出 .md」存成 Markdown 文件。精选、全部动态与星标三个信息流可按当前领域、分类与检索条件整份导出，单次最多 200 条，且始终从第一条开始，与当前翻到第几页无关。
